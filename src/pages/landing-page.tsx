...
function AuthBrowser({ mode, authenticated, user, onMode, onSuccess, onLogout }: { mode: AuthMode; authenticated: boolean; user?: { name: string; email: string } | undefined; onMode: (mode: AuthMode) => void; onSuccess: () => void; onLogout: () => void }) {
  ...
  return (
    <div className="@container h-[calc(100%-2.65rem)] overflow-auto bg-[#d9dccd]">
      ...
      {(page === 'login' || page === 'register') && <div className="grid min-h-[30rem] grid-cols-[minmax(12rem,.65fr)_minmax(20rem,1.35fr)] bg-[#f1f0e4] @max-[620px]:grid-cols-1">
...
export function LandingPage() {
  ...
  return (
    <main className="desktop-scanlines relative min-h-dvh overflow-hidden bg-[#756bb6] bg-[url('/assets/Home/Background.png')] bg-cover bg-center text-white [text-shadow:2px_2px_0_#171426]" ref={desktopRef} tabIndex={-1}>
      ...
      {windows.welcome && !windows.welcome.minimized && (
        <DesktopWindow className="right-[4%] bottom-[12%] w-[min(68vw,52rem)] max-lg:right-[2%] max-lg:bottom-[14%] max-lg:w-[min(80vw,50rem)] max-sm:right-2 max-sm:bottom-20 max-sm:w-[calc(100vw-1rem)]" constraints={desktopRef} draggable={false} onActivate={() => activate('welcome')} onClose={() => close('welcome')} onMinimize={() => undefined} overflowVisible showMinimize={false} title="Selamat Datang" zIndex={windows.welcome.zIndex}>
          <div className="relative min-h-80 overflow-visible bg-[radial-gradient(circle_at_30%_25%,#252936,#11131b_70%)] text-white">
            <div className="w-[72%] p-[clamp(2rem,5vw,4rem)] [text-shadow:2px_2px_0_#000] max-sm:w-[78%] max-sm:p-5"><h1 className="mb-4 text-[clamp(1.2rem,2.2vw,1.8rem)] font-black leading-tight">{session.data ? `Selamat datang kembali, ${session.data.user.name}!` : 'Hai! Selamat datang di dunia digital.'}</h1><p className="text-[clamp(.95rem,1.5vw,1.2rem)] leading-relaxed">{session.data ? 'Semua permainan sudah terbuka. Pilih ikon game di desktop dan lanjutkan latihan keamanan digitalmu.' : 'Di sini, tidak semua pesan, file, dan permintaan informasi bisa langsung dipercaya. Siap menguji seberapa jeli kamu menjaga diri tetap aman?'}</p>{!session.data && <div className="mt-5 flex flex-wrap gap-3"><button className={dialogButton} onClick={() => { setAuthMode('login'); activate('browser'); }} type="button">Login</button><button className={dialogButton} onClick={() => { setAuthMode('register'); activate('browser'); }} type="button">Buat Akun</button></div>}</div>
            <img alt="Maskot JEJAK" className="absolute -right-14 -bottom-20 z-20 w-[min(38%,19rem)] max-sm:-right-8 max-sm:-bottom-14 max-sm:w-[42%]" src={session.data ? '/assets/Shared/Mascots/Mascot_Shocked.png' : '/assets/Shared/Mascots/Mascot_Busy.png'} />
          </div>
        </DesktopWindow>
      )}

      {windows.browser && <DesktopWindow className="top-[8%] left-[23%] w-[min(72vw,60rem)] max-lg:left-[16%] max-lg:w-[min(80vw,55rem)] max-sm:top-52 max-sm:left-2 max-sm:w-[calc(100vw-1rem)]" constraints={desktopRef} minimized={windows.browser.minimized} onActivate={() => activate('browser')} onClose={() => close('browser')} maximizable maximized={windows.browser.maximized} onMinimize={() => minimize('browser')} onToggleMaximize={() => toggleMaximize('browser')} resizable title="Browser" titleIcon="/assets/Desktop/IconBrowser.png" zIndex={windows.browser.zIndex}><AuthBrowser authenticated={Boolean(session.data)} mode={authMode} onLogout={() => void logout()} onMode={setAuthMode} onSuccess={authSuccess} user={session.data?.user ? { name: session.data.user.name, email: session.data.user.email } : undefined} /></DesktopWindow>}
      ...
      {windows.profile && <DesktopWindow className="top-[20%] left-[39%] w-[min(90vw,31rem)]" constraints={desktopRef} minimized={windows.profile.minimized} onActivate={() => activate('profile')} onClose={() => close('profile')} onMinimize={() => minimize('profile')} title="Profile" titleIcon="/assets/Desktop/IconProfile.png" zIndex={windows.profile.zIndex}><div className="grid min-h-56 grid-cols-[6rem_1fr] gap-4 p-5"><img alt="Avatar pemain" className="w-24" src="/assets/Shared/Mascots/Mascot_Happy.png" /><div><p className="mb-1 text-sm">PLAYER PROFILE</p><h2 className="mb-3 text-2xl">{session.data?.user.name ?? 'Guest Player'}</h2><dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2"><dt>Status</dt><dd>{session.data ? 'Online' : 'Belum login'}</dd><dt>Email</dt><dd className="truncate">{session.data?.user.email ?? '—'}</dd></dl>{session.data ? <button className={`${dialogButton} mt-4`} onClick={() => void logout()} type="button">Logout</button> : <button className={`${dialogButton} mt-4`} onClick={() => activate('browser')} type="button">Buka Browser untuk Login</button>}</div></div></DesktopWindow>}
